   ...
   # logging.info(f"RSSI: {rssi_device_connected}")
    ...
    if fecha_alarm is None and (alarm_rms != 0 or alarm_vel != 0):
        ...
        data_alarm = {
            "alarm_rms": alarm_rms,
            "alarm_vel": alarm_vel,
            "fecha": fecha_alarm.strftime('%Y-%m-%d %H:%M:%S') if fecha_alarm else None,
            "med_x": alarm_med_x,
            "med_y": alarm_med_y,
            "med_z": alarm_med_z,
...
async def send_commands_ce(array_50, client):
    ...
    try:
        ...
        for idx in ones_idx_dev:
            ...
            command_ce = COMMAND_CE + bytearray([idx])
            ...
            try:
                ...
                logging.info(
                    f"----- 🧾 Solicitud de descarga de medicion índice {idx}")
            except BleakError as e:
                logging.error(
                    f"***** 🚨 Error al solicitar medicion índice {idx}: {e}")
    ...
    if device:
        ...
        while retries < MAX_RETRIES and not success:
            ...
            try:
                ...
                await asyncio.sleep(0.2)  # Espera antes de continuar
